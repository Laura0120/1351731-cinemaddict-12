...
const renderContent = () => {
  const filmslistContainers = siteMainElement.querySelectorAll(`.films-list__container`);
  const filmslistContainer = filmslistContainers[0];
  const topRatedContainer = filmslistContainers[1];
  const mostCommentedContainer = filmslistContainers[2];

  for (let i = 0; i < FILM_CARD_COUNT; i++) {
    render(filmslistContainer, createFilmCard(), `beforeend`);
  }

  render(filmslistContainer, createButtonShowMore(), `afterend`);
